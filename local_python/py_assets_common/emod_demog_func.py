...
def demog_vd_over(ref_name, node_list, cb_rate,
                  mort_year, mort_mat, age_x_list, age_y=None, idx=0,
                  mort_age_in=None):
    ...
    # Sanitize ages to ensure strictly increasing
    for k1 in range(1, len(age_x_list)):
        if (age_x_list[k1] <= age_x_list[k1-1]):
            age_x_list = age_x_list[:k1]
            age_y_list = age_y_list[:k1]
            break
    ...
    ind_age = AgeDistribution(ages_years=age_y_list,
                              cumulative_population_fraction=age_x_list)
...
def demog_is_over(ref_name, node_list, R0, age_x, age_y=None, idx=0):
    ...
    isus_y = [round(np.minimum(np.exp(iSP0*(val/365.0-0.65)), 1.0), 4)
              for val in isus_x]
    isus_x_yr = [val/365.0 for val in isus_x]

    # Initial susceptibility distribution
    ind_sus = SusceptibilityDistribution(ages_years=isus_x_yr,
                                         susceptible_fraction=isus_y)
